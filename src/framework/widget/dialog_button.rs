...
/// 使用组件图片绘制，支持按下和悬停状态
///
/// [TRANSLATION_NOTE]: C++ DialogButton 继承 Widget；Rust 改为组合：首字段内嵌
/// `Widget` 基座实现布局对齐（`&mut self.widget as *mut Widget` 可安全挂接到
/// WidgetManager），其余字段通过 Deref/DerefMut 转发使调用点 `db.x` 等不变。
#[repr(C)]
pub struct DialogButton {
    /// Widget 基座（对应 C++ 基类成员，含 x/y/width/height/状态/颜色等）
    pub widget: Widget,
    pub label: String,
    pub label_justify: i32,
    pub font: Option<Box<Font>>,
    pub button_image: *mut Image,
    // 对应 C++ mOverImage/mDownImage/mDisabledImage（NewLawnButton::Draw 使用）
...
impl std::ops::Deref for DialogButton {
    type Target = Widget;
...
impl std::ops::DerefMut for DialogButton {
...
impl DialogButton {
    /// 获取可挂接到 WidgetManager 的 Widget 基座指针
    /// [TRANSLATION_NOTE]: C++ 中按钮作为 Widget 子类直接挂接；Rust 组合后
    /// 通过首字段基座的 field projection 获得指针对应，无需布局假设
    pub fn as_widget_ptr(&mut self) -> *mut Widget {
        &mut self.widget as *mut Widget
    }
    ...
    pub fn new(
        ...
        // Widget 基座默认值（对应 C++ DialogButton 构造中的 mHasAlpha=true 等）
        db.has_alpha = true;
        db.clip = true;
        db.visible = true;
        db.do_finger = true;
        // 设置默认颜色
        db.colors = G_DIALOG_BUTTON_COLORS
            .iter()
