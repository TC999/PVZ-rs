...
pub struct ReanimatorTrackInstance {
    ...
    // 对应 C++ mBlendCounter/mBlendTime/mBlendTransform（StartBlend 记录混合源）
    pub m_blend_count: f32,
    pub m_blend_time: i32,
    pub m_blend_transform: ReanimatorTransform,
    pub m_shake_x: f32,
    pub m_shake_y: f32,
    // 对应 C++ mShakeOverride（SetShakeOverride 写入的显式震动）
    pub m_shake_override: f32,
    // 对应 C++ mTruncateDisappearingFrames（SetTruncateDisappearingFrames 写入）
...
impl ReanimatorTrackInstance {
    pub fn new() -> Self {
        ReanimatorTrackInstance {
            ...
            m_blend_count: 1.0,
            m_blend_time: 0,
            m_blend_transform: ReanimatorTransform::default(),
            m_shake_x: 0.0,
            m_shake_y: 0.0,
