...
这些条目是自动生成的 C++ 原始代码转储。对应的枚举已在上面标记为 `[x]`，
Rust 实现在 rust/src/lawn/game_enums.rs 中已完整翻译（所有枚举常量导入为 Rust enum 变体）。

- `[x]` 所有枚举定义（AdviceType, AlmanacPage, AwardType 等共 68 个枚举）已在 game_enums.rs 中实现
  - AdviceType（68 值）→ game_enums.rs:74
  - AlmanacPage（3 值）→ game_enums.rs 中对应位置
  - AwardType（5 值）→ game_enums.rs 中对应位置
  - BackgroundType（10 值）→ game_enums.rs 中对应位置
...
当前进度：约 88% 完成（~3500 行 Rust / 9857 行 C++）

已翻译的方法（约 193 个）：
- 结构体字段：基本完整
- 初始化：new(), board_init(), init_level()
- 生命周期：dispose_board(), try_to_save_game(), complete_end_level_sequence_for_saving() [存根]
- 背景资源：load_background_images() [本轮新增，简化版]
- 关卡初始化：init_lawn_mowers(), start_level() [本轮新增]
- 更新循环：update(), update_sun_spawning(), update_waves(), spawn_zombie_wave(), check_collisions()
- 绘制：draw(), draw_background(), draw_ui()
...
- 行/种子：row_can_have_zombies(), row_can_have_zombie_type(), get_num_seeds_in_bank(), choose_seeds_on_current_level() [本轮升级为完整版], get_seed_packet_position_x(), is_plant_in_cursor(), get_seed_bank_extra_width()
- 静态方法：is_zombie_type_pool_only(), is_zombie_type_spawned_only(), can_zombie_spawn_on_level()
- 生存/关卡辅助：get_num_waves_per_survival_stage(), get_survival_flags_completed()
...
- 效果：shake_board(), is_ice_at(), can_add_bob_sled(), freeze_effects_for_cutscene() [本轮新增存根]
...
- 僵尸生成：add_zombie_in_row(), add_zombie(), pick_row_for_new_zombie(), total_zombies_health_in_wave(), pick_zombie_waves(), pick_zombie_type(), put_zombie_in_wave(), put_in_missing_zombies(), init_zombie_waves(), is_zombie_wave_distribution_ok()
...
本轮设计决策：
- C++ GAMEMODE_UPSELL/GAMEMODE_INTRO/CHALLENGE_RESODDED 在 Rust GameMode 中暂缺，已注释
- FoleyType::FOLEY_RAIN → FoleyType::Rain，内联引用避免导入冲突
- ZenGarden.board 是 Option<*mut Board>，清理时用 Some(null_mut())
- LawnMower 使用 Vec<LawnMower> + push 替代 C++ DataArrayAlloc
编译状态: cargo check 通过
```
...
待翻译的主要模块（按优先级排序）：
1. ~~DrawBackdrop / DrawLevel / DrawGameObjects（完整绘制逻辑）~~ → 当前正在翻译
...
- update_grid_items_detailed()：从简化版升级为完整版，新增弹坑计数器递减/消失、mEnablingGraveStones 检查、GridItem::update() 调用
- update_fwoosh()：火焰扫荡效果（暂不完整实现）

本轮新增翻译（当前轮次）：
- update_grid_items_detailed()：补全了 C++ Board::UpdateGridItems 的全部逻辑
  - 墓碑计数器递增仅在 m_enable_grave_stones 为 true 且 counter < 100 时进行
  - 弹坑在 Playing 场景下递减，到 0 时调用 grid_item_die()
  - 每个 GridItem 调用 update() 逐帧更新
- grid_item_die()：从 /* */ 注释块中移出，设为 pub 方法，设置 dead = true

本轮新增翻译（当前轮次）：
- draw_fade_out()：完整实现（对应 C++ DrawFadeOut）
  - 检查 m_board_fade_out_counter >= 0 且非 survival with repick
  - 使用 TodAnimateCurve 计算淡出透明度
  - 第 10/20/30/40/50 关使用黑色淡出，其余关卡使用白色淡出
  - 调用 Graphics::fill_rect_xywh 填充全屏

本轮新增翻译（当前轮次）：
- update_fog()：完整实现（对应 C++ UpdateFog）
  - 雾的恢复速度依 m_fog_blown_count_down 分三级（20/1/3）
  - 左列初始雾量上限 200，其余列 255
  - 灯笼草（Plantern）驱散周围 4 格、火炬树桩（Torchwood）驱散 1 格
  - 使用 NotOnGround 等效检查（dead/squished/bungee）过滤植物
- clear_fog_around_plant()：完整实现（对应 C++ ClearFogAroundPlant）
  - 支持大小 4（灯笼）和 1（火炬）的菱形驱散范围
  - 大小 4 时有特殊的裁剪逻辑（distX>3 || distY>2 排除，distX+distY==5 排除）
  - 雾的消退速度同样依 m_fog_blown_count_down 分三级
  - Rust 借用安全：先收集位置信息再遍历修改 grid_cel_fog
- update() 中新增 update_fwoosh() 调用链，与 C++ Board::Update() 调用顺序一致

本轮新增字段：
- m_fwoosh_count_down: i32 — 火焰扫荡倒计时（C++ mFwooshCountDown）
- m_fwoosh_id: [[ReanimationID; 12]; 6] — 火焰扫荡动画 ID 二维数组（C++ mFwooshID）
  - 字段已添加并在 new() 中初始化，update_fwoosh() 完整实现待 EffectSystem 提供 Reanimation 查找方法后再补全
```
...
- `[x]` `class Coin : GameObject` (L17, 0 个方法, 10 个成员)

**翻译备注:**

```
翻译文件: rust/src/lawn/coin.rs
Coin 结构体已完整翻译，包含 new(), coin_initialize(), update(), draw(), collect() 等方法。
struct 使用 base: GameObject 组合模式替代 C++ 继承。
```
...
- `[x]` `enum PlantSubClass` → { SUBCLASS_NORMAL, SUBCLASS_SHOOTER }
...
- `[x]` `class ZombieDrawPosition` (L66, 0 个方法, 7 个成员)
- `[x]` `class Zombie : GameObject` (L81, 0 个方法, 10 个成员)
- `[x]` `class ZombieDefinition` (L401, 0 个方法, 7 个成员)

**枚举:**

- `[x]` `enum ZombieAttackType` → { ATTACKTYPE_CHEW, ATTACKTYPE_DRIVE_OVER, ATTACKTYPE_VAULT, ATTACKTYPE_LADDER }
- `[x]` `enum ZombieParts` → { PART_BODY, PART_HEAD, PART_HEAD_EATING, PART_TONGUE, PART_ARM, ... (13 个值) }
...
```
翻译文件: rust/src/todlib/data_array.rs
泛型 DataArray<T> 结构体已实现。DataArrayItem 类型在泛型实现中内联处理。
```
...
- `[x]` `enum FilterEffect` → { FILTER_EFFECT_NONE, FILTER_EFFECT_WASHED_OUT, FILTER_EFFECT_LESS_WASHED_OUT, FILTER_EFFECT_WHITE, NUM_FILTER_EFFECTS }

**翻译备注:**

```
翻译文件: rust/src/todlib/filter_effect.rs
FilterEffectType 枚举已实现（命名略有不同）。
```
...
- `[x]` `class ReanimAtlasImage` (L17, 0 个方法, 5 个成员)
- `[x]` `class ReanimAtlas` (L32, 0 个方法, 2 个成员)

**翻译备注:**

```
翻译文件: rust/src/todlib/reanim_atlas.rs
ReanimAtlasImage 和 ReanimAtlas 结构体已实现。
```
...
- `[x]` `class TodListNode` (L32, 0 个方法, 3 个成员)
- `[x]` `class TodList` (L40, 0 个方法, 10 个成员)

**翻译备注:**

```
翻译文件: rust/src/todlib/tod_list.rs
TodListNode<T> 和 TodList<T> 泛型结构体已实现。
缺失: TodAllocator 结构体未实现。
```
...
- `[x]` `class FloatParameterTrackNode` (L74, 0 个方法, 5 个成员)
- `[x]` `class FloatParameterTrack` (L89, 0 个方法, 2 个成员)
- `[x]` `class ParticleField` (L101, 0 个方法, 3 个成员)
- `[x]` `struct EmitterFieldArray` (L109, 0 个方法, 2 个成员)
- `[x]` `class TodEmitterDefinition` (L121, 0 个方法, 10 个成员)
- `[x]` `class TodParticleDefinition` (L180, 0 个方法, 2 个成员)
- `[x]` `class ParticleParams` (L195, 0 个方法, 2 个成员)
- `[x]` `class TodParticleHolder` (L253, 0 个方法, 5 个成员)
- `[x]` `class ParticleRenderParams` (L272, 0 个方法, 10 个成员)
- `[x]` `class TodParticle` (L294, 0 个方法, 10 个成员)
- `[x]` `class TodParticleEmitter` (L315, 0 个方法, 10 个成员)
- `[x]` `class TodParticleSystem` (L363, 0 个方法, 8 个成员)

**枚举:**

- `[x]` `enum ParticleFlags` → { PARTICLE_RANDOM_LAUNCH_SPIN, PARTICLE_ALIGN_LAUNCH_SPIN, PARTICLE_ALIGN_TO_PIXELS, PARTICLE_SYSTEM_LOOPS, PARTICLE_PARTICLE_LOOPS, ... (12 个值) }
- `[x]` `enum ParticleFieldType` → { FIELD_INVALID, FIELD_FRICTION, FIELD_ACCELERATION, FIELD_ATTRACTOR, FIELD_MAX_VELOCITY, ... (13 个值) }
- `[x]` `enum ParticleSystemTracks` → { TRACK_SPAWN_RATE, TRACK_SPAWN_MIN_ACTIVE, TRACK_SPAWN_MAX_ACTIVE, TRACK_SPAWN_MAX_LAUNCHED, TRACK_EMITTER_PATH, ... (11 个值) }
- `[x]` `enum ParticleTracks` → { TRACK_PARTICLE_RED, TRACK_PARTICLE_GREEN, TRACK_PARTICLE_BLUE, TRACK_PARTICLE_ALPHA, TRACK_PARTICLE_BRIGHTNESS, ... (17 个值) }

**翻译备注:**

```
翻译文件: rust/src/todlib/tod_particle.rs
所有 12 个结构体和 4 个枚举已完整实现（含 update, draw 等方法）。
```
...
- `[x]` `class TodStringListFormat` (L17, 0 个方法, 5 个成员) — Rust 中名为 TodStringFormat
...
- `[x]` `class SexyAppBase : ButtonListener, DialogListener` (L123, 0 个方法, 10 个成员)
...
翻译文件: rust/src/framework/sexy_app_base.rs
SexyAppBase 结构体已实现。
缺失: WidgetSafeDeleteInfo 结构体未实现。
```
