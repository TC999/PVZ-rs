...
当前进度：约 90% 完成（~3520 行 Rust / 9857 行 C++）

已翻译的方法（约 196 个）：

本轮新增翻译（当前轮次）：
- complete_end_level_sequence_for_saving()：完整实现（对应 C++ L1850-L1880）
  - 遍历所有未触发割草机，将其转换为金币（简化版：每个 +100）
  - 将场上所有硬币标记为死亡（清理）
  - 注：C++ 中调用 UpdatePlayerProfileForFinishingLevel() 暂未实现
- do_planting_effects()：完整实现（对应 C++ L2101-L2143）
  - 根据植物类型计算粒子位置偏移（睡莲 +15，花盆 +30）
  - 温室/水族馆背景特殊音效
  - 水面种植 vs 地面种植不同粒子效果
  - 调用 play_foley() 和 add_tod_particle()（目前为空存根）
- Board 新增字段 m_coins_collected: i32（统计用）

待翻译的主要模块：
1. MouseDown/MouseUp/MouseDownWithPlant 完整版交互逻辑（依赖 CursorObject 字段补充）
2. UpdateToolTip / UpdateCursor 完整版
3. MouseHitTest / MouseHitTestPlant 点击检测
