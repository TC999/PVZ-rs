...
本轮新增翻译：
- mouse_down()：升级为完整版（对应 C++ MouseDown L4479-L4614）
  - 命中检测（mouse_hit_test 存根）
  - 委托 Challenge::MouseDown
  - 工具点击 vs 植物种植 vs 其他对象的分支逻辑
  - 调用 update_cursor() 更新光标
- mouse_up()：新增（对应 C++ MouseUp L4700-L4760）
  - 忽略标记逻辑（mIgnoreMouseUp）
  - 按钮交互检查
- update_cursor()：新增简化版（对应 C++ UpdateCursor L3008-L3103）
  - 暂停/淡出时跳过
  - 命中检测决定光标形状骨架
- mouse_hit_test()：新增存根（对应 C++ MouseHitTest L4225）
- mouse_down_with_tool()：新增存根（对应 C++ MouseDownWithTool L4099）
- mouse_down_with_plant()：新增存根（对应 C++ MouseDownWithPlant L3689）
- Board 新增字段 cursor_object: CursorObject

待翻译的主要模块：
1. UpdateToolTip 完整版（依赖 ToolTip widget 和资源常量系统）
2. MouseHitTest / MouseHitTestPlant 完整版（需要 GridItem/Zombie/Plant 坐标检测）
3. MouseDownWithPlant 完整版（种植逻辑）

依赖于 CursorObject 的字段（seed_bank_index, coin_id 等）已全部补全，不再阻塞。
