...
当前进度：约 93% 完成（~3570 行 Rust / 9857 行 C++）
...
- mouse_down_with_plant()：完整版（对应 C++ MouseDownWithPlant L3689-L4066）
  ...
  - 教程状态更新
  - 清除光标
- tool_hit_test()：新增（辅助方法，包装 MouseHitTest 做植物检测）
- Coin::mouse_hit_test() 新增（对应 C++ Coin::MouseHitTest L1428，含太阳扩展点击区域）

待翻译的主要模块：
1. MouseDownWithPlant 完整版（种植逻辑，依赖种植检查方法）
2. MouseHitTest 中缺的 GameObjectType 变体（Shovel/ScaryPot/SlotMachineHandle）待 Rust 枚举补充后启用
