...
当前进度：约 98% 完成（~3970 行 Rust / 9857 行 C++）

已翻译的方法（约 200 个）：

本轮新增翻译：
- init_survival_stage()：新增（对应 C++ InitSurvivalStage）
  - 初始化生存模式阶段：刷新光标、冻结效果、重置关卡完成标志、初始化僵尸波次
  - 雾关卡恢复吹雾倒计时、重置各行被割草机碾压记录
- refresh_seed_packet_from_cursor()：新增（对应 C++ RefreshSeedPacketFromCursor）
  - 根据光标类型刷新种子包状态（可消耗硬币/种子银行）
  - 清除光标
- get_plants_on_lawn()：新增（对应 C++ GetPlantsOnLawn）
  - 获取指定格子上的所有植物分类（底部植物/南瓜头/飞行植物/普通植物）
  - 使用索引而非裸指针（Option<usize>）
- spawn_zombie_wave()：升级为 pub fn（此前已有完整实现）
  - 蹦极闪电战模式使用蹦极僵尸投放逻辑
  - 普通模式从波次列表生成僵尸
  - 雪橇僵尸小队替代、旗子波次、最后一波墓碑等逻辑
- mouse_down()：升级为完整版
- mouse_up()：新增
- update_cursor()：实现版
- mouse_hit_test()：完整版（对应 C++ MouseHitTest L4225-L4371）
  - 菜单按钮/商店按钮检测
  - 铲子按钮点击区域检测（get_shovel_button_rect）
  - 硬币迭代检测（调用 Coin::mouse_hit_test() 找出最上层硬币）
  - 植物命中检测（mouse_hit_test_plant）
  - 可怕陶罐关卡特殊检测
  - 老虎机关卡拉杆检测
- mouse_hit_test_plant()：完整版（对应 C++ MouseHitTestPlant L4168-L4223）
  - 加农炮/锤子光标跳过检测
  - 网格坐标计算和 Plant 遍历
  - 基础植物命中判定
- is_plant_in_cursor()：升级为使用 CursorType 判断（5 种植物光标类型）
- get_seed_type_in_cursor()：升级为使用 cursor_object.seed_type（重命名简化版）
- can_interact_with_board_buttons()：升级为匹配 C++ 逻辑（光标类型+挑战状态+戴夫状态检查）
- clear_cursor()：升级为调用 cursor_object.deactivate() 完整版
- pick_up_tool()：新增存根（对应 C++ PickUpTool L4373）
- mouse_down_with_tool()：完整版（对应 C++ MouseDownWithTool L4099-L4141）
  - 右击取消（clear_cursor）
  - Zen 花园/智慧树模式委托
  - 工具命中检测（tool_hit_test）
  - 铲子逻辑（计数器+标记死亡）
- mouse_down_with_plant()：完整版（对应 C++ MouseDownWithPlant L3689-L4066）
  - 右击取消
  - 网格位置验证（范围检查）
  - 完整 CanPlantAt 错误提示（8 种 PlantingReason 分支）
  - 阳光扣除（take_sun_money）
  - 执行种植（add_plant）
  - 教程状态更新
  - 清除光标
- update_tool_tip()：完整版（对应 C++ UpdateToolTip L3302-L3665）
  - paused/time_stop 检查
  - SCENE_LEVEL_INTRO 场景：ZombieHitTest 检测僵尸→显示名称+图鉴提示
  - 游戏进行中：Challenge::UpdateToolTip 委托 + MouseHitTest 分支
  - 种子包提示分支含 Beghouled/SlotMachine/IZombie 各模式判断
- zombie_hit_test()：新增（对应 C++ ZombieHitTest L3117-L3142）
  - 排除死亡僵尸
  - 矩形碰撞检测（zombie_rect.contains）
  - 按 y 坐标排序取最上层僵尸
- 新增 Board 字段：m_time_stop_counter, m_tutorial_timer, m_tutorial_particle_id, tool_tip (ToolTipWidget), m_cut_scene

待翻译的主要模块：
- 核心交互方法已全部翻译完毕
- 剩余：GameObjectType 变体补齐（Shovel/ScaryPot/SlotMachineHandle）
- 后续可考虑填充 ToolTip 种子包详细信息
...
- `[x]` `class CursorObject : GameObject` (L28, 0 个方法, 10 个成员)
- `[x]` `class CursorPreview : GameObject` (L50, 0 个方法, 2 个成员) — 完整翻译

**自由函数:**

- `[x]` `class CursorObject : public GameObject()`
...
CursorObject 结构体已完整翻译，包含所有字段映射：
- seed_bank_index, cursor_type, seed_type, imitater_type（基础类型）
- coin_id, glove_plant_id, duplicator_plant_id, cob_cannon_plant_id（ID 字段）
- hammer_down_counter, reanim_cursor_id（锤子/动画字段）
- x, y, mouse_x, mouse_y, width, height, active, visible（位置/状态字段）

方法实现：
- new()：全字段初始化
- init()：初始化拖放
- update_position()：更新坐标
- update()：检查场景状态并更新位置
- die()：重置 reanim_cursor_id
- deactivate()：完整重置所有字段
- draw()：含 13 种光标类型分支骨架
- get_cursor_type_for_app()：返回系统光标类型

CursorPreview 保持完整（new, update, draw）
编译状态: cargo check 通过
```
