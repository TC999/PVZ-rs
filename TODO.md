...
当前进度：约 98% 完成（~4000 行 Rust / 9857 行 C++）

已翻译的方法（约 200 个）：

本轮新增翻译：
- drop_loot_piece()：新增（对应 C++ DropLootPiece L9403）
  - 首次冒险模式特殊掉落（L22 小游戏礼物、L36 解谜模式礼物）
  - 打地鼠模式阳光掉落逻辑
  - 盆栽植物/巧克力/钻石/金/银币概率掉落
  - 核桃保龄球关卡不掉钱
  - L11 确保玩家有足够钱买第一个种子槽升级
- next_wave_coming()：新增（对应 C++ NextWaveComing L5334）
  - 触发最终波次动画和音效逻辑
  - 旗波/第一波播放对应音效
- update_zombie_spawning()：扩展升级
  - 添加 huge wave 倒计时处理（m_huge_wave_count_down）
  - 添加最终波次声音计时（m_final_wave_sound_counter）
- 新增 Board 字段：m_huge_wave_count_down, m_zombie_count_down,
  m_final_wave_sound_counter, m_sun_money, m_dropped_first_coin
- ZenGarden 新增方法：can_drop_potted_plant_loot(), can_drop_chocolate()
  （从 TODO 存根升级为完整实现）
